...
function renderHtml(name, data, res) {
    ...
    if (!compiled[name]) {
        if (compiled["error"]) {
            data.message = "Sorry, we could not find the proper template to render this data.";
            res.end(compiled["error"](data));
        } else {
           console.log("Missing default error template");
           res.end("Sorry, we couldn't find the template to render your data. But, here is your data in JSON format: <br><br>" + JSON.stringify(data));
        }
    } else {
