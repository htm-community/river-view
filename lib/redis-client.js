 ...
 * @class RedisClient
 */
function RedisClient(url, maxDataPointsPerRequest, defaultSnapTo) {
    this.url = url;
    this.maxDataPointsPerRequest = maxDataPointsPerRequest;
    this.defaultSnapTo = defaultSnapTo;
}
 ...
 * @param riverName {string} River name
 * @param options {Object} Any additional query options, currently only contains
 *                         paging information.
 * @param options.page {int} Page number to regtrieve (default is 1).
 * @param options.itemsPerPage {int} How many items should be on each page.
 * @param callback {function} handles response
 */
RedisClient.prototype.getKeys = function getKeys(riverName, options, callback) {
    ...
    this.client.keys(riverName + ':*:data', function(error, keys) {
        ...
        cleanKeys = cleanKeys.sort();
        if (options && options.itemsPerPage && options.page) {
            // Only send one page if paging options are given.
            cleanKeys = _.chunk(cleanKeys, options.itemsPerPage)[options.page - 1];
        }
...
RedisClient.prototype.getRiverData =
    function getRiverData(riverName, streamId, query, callback) {
        var me = this, key, since, until, limit, snapto;
        if (!query) {
            query = {};
        }
        ...
        snapto = query.snapto || this.defaultSnapTo;
        key = riverName + ':' + streamId + ':data';

        if (since !== '-inf') {
            // Make the since exclusive.
            since = '(' + since;
        }

        function snapDataLeft(key, since, until, limit, localCallback) {
            var fetchArgs = [
                key, since, until,
                'WITHSCORES',
                'LIMIT', 0, limit
            ];
            me.client.zrangebyscore(fetchArgs, function(error, values) {
                if (error) return callback(error);
                localCallback(null, unzipScores(values));
            });

        }

        function snapDataRight(key, since, until, limit, localCallback) {
            var fetchArgs = [
                key, until, since,
                'WITHSCORES',
                'LIMIT', 0, limit
            ];
            me.client.zrevrangebyscore(fetchArgs, function(error, values) {
                if (error) return callback(error);
                localCallback(null, unzipScores(values).reverse());
            });
        }

        // User-defined 'since', no until
        if (since !== '-inf' && until == '+inf') {
            snapDataLeft(key, since, until, limit, callback);
        }
        // User-defined 'until', no since
        else if (since == '-inf' && until !== '+inf') {
            snapDataRight(key, since, until, limit, callback);
        }
        // User-defined both 'since' and 'until', so we need to refer to the
        // 'snapto' value.
        else if (since !== '-inf' && until !== '+inf') {
            if (snapto == 'since') {
                snapDataLeft(key, since, until, limit, callback);
        ...
        // Otherwise, both since/until are infinity, so snap to right
        else {
            snapDataRight(key, since, until, limit, callback);
        }
