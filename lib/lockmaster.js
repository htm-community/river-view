...
Lockmaster.prototype.start = function start() {
    ...
    _.each(me.rivers, function(river) {
        ...
        me.redisClient.logActivity(
            'Starting river "' + name + '", which will run every '
          + river.config.interval + ' seconds.'
        );
        ...
        riverRunner = function() {
            ...
            _.each(config.sources, function(url) {
                ...
                me.redisClient.logActivity(river.name + ' request to ' + url);
                request.get(url, function(err, resp, body) {
                    if (err) {
                        console.warn(
                            'HTTP error while connecting to %s\n' +
                            'Erro:\n%s\n' +
                            'Continuing...'
                          , url, err.message
                        );
                    ...
                    me.redisClient.logActivity(
                        river.name + ' response '
                      + resp.statusCode + ' received from ' + url
                    );
