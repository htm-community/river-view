...
Lockmaster.prototype.preLoadSodaRiver = function(river, callback) {
    ...
    _.each(config.soda, function(sodaConfig) {
        ...
        var dataCutoff;
        var expirationDivisor = 2;
        var streamName = sodaConfig.name;
        var preLoadLimit = SODA_PRELOAD_LIMIT;

        // Users can specify up to 2 years of data storage for a river and its
        // streams. If they have a long period (over a year), we don't want to
        // preload as aggressively.
        if (expiresDuration.asYears() >= 1) {
            expirationDivisor = 32;
        }

        dataCutoff = moment().subtract(expiresDuration.asDays() / expirationDivisor, 'days');
        ...
        loaders[riverName + ' ' + streamName] = function(localCallback) {
            ...
            // If the river name is the same as the stream name, it means that
            // the sode config only has one stream defined in it, and the stream
            // creation (if it is happening) is being done in the parser.
            if (streamName && streamName !== river.name) {
...
Lockmaster.prototype.initializeRivers = function(callback) {
    ...
    async.parallel(timeFetchers, function(err, times) {
        ...
        async.parallel(initializers, function(err, sourceUrls) {
            ...
            console.log('All Rivers have been initialized.');
