...
River.prototype.getDataCount = function(id, callback) {
    this.redisClient.getDataCount(this.name, id, callback);
};

River.prototype.getFirstAndLastUpdatedTimes = function(id, callback) {
    var me = this,
        riverName = me.name,
        timezone = me.config.timezone;
    me.redisClient.getLatestTimestampForRiverData(riverName, id, function(err, timestamp) {
        var latest;
        if (err) return callback(err);
        latest = moment.unix(timestamp).tz(timezone);

        me.redisClient.getEarliestTimestampForRiverData(riverName, id, function(err, timestamp) {
            var earliest;
            if (err) return callback(err);
