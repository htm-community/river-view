...
function augmentWithNavigationDetails(payload, query, firstPoint, lastPoint) {
    ...
    if (query.limit == undefined && payload.meta.count < CONFIG.maxDataPointsPerRequest) {
        // In this case, user defined no limit, and the number of data points
        // within the given date range was below the default limit, so we should
        // create since/until times that reflect the original query because no
        // data was left out. This is the most common case.
