...
function renderPage(name, data, format, res) {
    if (format == 'json') {
        ...
        renderError(
            new Error('Format ' + format + ' is unsupported!')
          , 'html', res
        );
    }
}
...
function handleLogRequest(req, res) {
    redisClient.getLogs(function(error, logs) {
        if (error) return renderError(error, 'html', res);
        renderPage('activity', {
            logs: logs
...
function handleRiverMetaDataRequest(req, res) {
    ...
    renderPage('riverMeta', data, ext, res);
}
...
function handleTemporalDataRequest(req, res) {
    ...
    if (! river) {
        ...
        return renderError(
            new Error('River "' + riverName + '" does not exist.'), 'html', res
        );
    }
...
function handle404(req, res) {
    ...
    // respond with json
    if (req.accepts('json')) {
        renderError(new Error(message), 'json', res);
        return;
    }
