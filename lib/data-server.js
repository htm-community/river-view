...
function augmentWithNavigationDetails(payload) {
    var out = _.extend({}, payload),
        baseurl = CONFIG.baseurl + '/' + payload.name + '/' + payload.id + '/data.json',
        since = payload.meta.since.timestamp,
        until = payload.meta.until.timestamp,
        payloadDuration = until - since,
        leftBound = since,
        rightBound = until,
    ...
    out.urls = {
        prev: encodeURI(baseurl + '?since=' + prevBound + '&until=' + leftBound),
        next: encodeURI(baseurl + '?since=' + rightBound + '&until=' + nextBound)
    };
...
function handleTemporalDataRequest(req, res) {
    ...
    river.getTemporalData(id, query, function(error, data) {
        ...
        payload = augmentWithNavigationDetails(payload);
