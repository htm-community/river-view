        ...
        if (ext == 'json' && includeDetails) {
            _.each(keys, function(key) {
                propertyFetchers[key] = function(callback) {
                    redisClient.getRiverMetaData(riverName, key, function(err, props) {
                        if (err) return callback(err);
                        callback(null, JSON.parse(props));
                    });
                };
            });
            async.parallel(propertyFetchers, function(err, props) {
...
function startDataService(opts) {
    ...
    app.use('/:river/props\.:ext?', handleRiverPropertiesRequest);
    app.use('/:river/keys\.:ext?', handleRiverKeys);
    app.use('/:river/:id/data\.:ext?', handleTemporalDataRequest);
    app.use('/:river/:id/props\.:ext?', handleMetaDataRequest);
